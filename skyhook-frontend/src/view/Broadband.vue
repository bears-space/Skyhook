...
const { bb, meta, system, overallHealth } = storeToRefs(comms)
...
const fmtInt = (n: number | null): string => (n == null ? NA : `${Math.round(n)}`)
...
const addSample = (
  seriesRef: typeof downlinkKbps,
  catRef: typeof downlinkCategories,
...
watch(
  () => [bb.value.down.rateBps.ts, bb.value.down.rateBps.value],
  ([ts, val]) => addSample(downlinkKbps, downlinkCategories, val, ts),
  { immediate: true },
)

const downlinkSeries = computed(() => [
  { name: "Broadband", type: "area", data: downlinkKbps.value },
  { name: "Broadband", type: "line", data: downlinkKbps.value },
])
...
const downlinkMetrics = computed<Metric[]>(() => {
  const m = meta.value.bb.down
  const f = bb.value.down
  return [
    ...
    { key: "mcs", label: "MCS", value: fmtInt(f.mcs.value), meta: m.mcs },
    { key: "phy", label: "PHY rate", value: fmtBps(f.phyRateBps.value), meta: m.phyRateBps },
    ...
    { key: "channel", label: "Channel", value: fmtInt(f.channel.value), meta: m.channel },
...
const bbDownHealthText = computed(() => (hasData(meta.value.bb.down.health) ? bb.value.down.health.value : NA))
const bbDownStateText = computed(() => (hasData(meta.value.bb.down.state) ? bb.value.down.state.value : NA))

const headroomLabel = computed(() => {
  const phy = bb.value.down.phyRateBps.value
  const rate = bb.value.down.rateBps.value
  if (phy == null || rate == null || phy <= 0) return NA
  const headroom = Math.max(0, phy - rate)
  const pct = (headroom / phy) * 100
  if (!Number.isFinite(pct)) return NA
  return `Headroom ${pct.toFixed(0)}%`
})
</script>
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <div class="text-lg font-semibold">Broadband</div>
        <div class="text-sm text-muted-foreground">
          Live link health for the broadband downlink channel.
          <span class="font-medium text-foreground">
            Updated {{ hasData(lastUpdateMeta) ? fmtAge(lastUpdateMeta) : NA }} ({{ systemSource }}).
          </span>
        </div>
        <div v-if="systemNotes !== NA" class="mt-1 text-xs text-muted-foreground">
          Notes: <span class="text-foreground">{{ systemNotes }}</span>
        </div>
      </div>
      <div class="flex flex-wrap gap-2">
        <Badge variant="outline" :class="toneClass(healthTone(overallHealthText))">
          Overall {{ overallHealthText }}
        </Badge>
        <Badge variant="outline" :class="toneClass(healthTone(bbDownHealthText))">
          Downlink: {{ bbDownHealthText }}
        </Badge>
        <Badge variant="outline" :class="toneClass(stateTone(bbDownStateText))">
          {{ stateLabel(bbDownStateText) }}
        </Badge>
    ...
    <div class="grid gap-4">
      <Card class="shadow-sm">
        <CardHeader class="pb-2 space-y-1">
          ...
          <CardDescription>High-rate telemetry/video return path.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-4">
          <div class="flex flex-wrap items-center gap-3">
            <div class="text-3xl font-semibold leading-tight">
              {{ fmtBps(bb.down.rateBps.value) }}
            </div>
            <Badge variant="outline" :class="toneClass(staleTone(meta.bb.down.rateBps))">
              {{ liveLabel(meta.bb.down.rateBps) }}
            </Badge>
            <Badge variant="outline" class="bg-secondary/60 text-foreground">
              Avg {{ fmtBps(bb.down.rateAvgBps.value) }}
            </Badge>
            <Badge variant="outline" class="bg-secondary/60 text-foreground">
              Util {{ fmtPct(bb.down.utilPct.value, 0) }}
            </Badge>
            <Badge variant="outline" class="bg-secondary/60 text-foreground">
              {{ headroomLabel }}
            </Badge>
          ...
          >
            <Badge class="mt-2" :class="toneClass(staleTone(meta.bb.down.rateBps))">
              {{ lastUpdateLabel(meta.bb.down.rateBps) }}
            </Badge>
          </GradientLineApex>
